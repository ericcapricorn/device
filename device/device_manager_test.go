...
func TestGetAllDevice(t *testing.T) {
	...
	devList, err := device.GetAllDevices(invalidHid)
	if err != nil {
	...
	for _, home := range list {
		devList, err := device.GetAllDevices(home.hid)
		if err != nil {
		...
		for _, dev := range devList {
			temp, err := device.Get(dev.GetDid())
			if err != nil {
...
func TestDeleteAll(t *testing.T) {
	...
	for _, home := range list {
		...
		for _, dev := range devList {
			...
			if dev.IsMasterDevice() {
				err := device.DeleteDevice(home.hid, dev.GetDid())
				if err != nil {
	...
	// invalid did
	var invalidDid int64
	err = device.DeleteDevice(invalidHid, invalidDid)
	if err != nil {
...
func TestChangeDeviceInfo(t *testing.T) {
	...
	for _, home := range list {
		...
		for _, dev := range devList {
			if !dev.IsMasterDevice() {
				...
				err = device.ChangeDeviceInfo(dev.GetDid(), "name", name)
				if err != nil {
					t.Error("change device info failed", dev.GetDid(), err)
				}
			} else {
				// be frozen
				err = device.ChangeDeviceStatus(dev.GetDid(), FROZEN)
				if err != nil {
	...
	err = device.ChangeDeviceInfo(invalidDid, "master_did", invalidDid)
	if err == nil {
